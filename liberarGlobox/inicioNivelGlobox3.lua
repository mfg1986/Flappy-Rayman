...
--Funcion para mover el fondo, los electroons y a rayman en la escena
 ...
 --Creamos un desplazamiento 
  ...
  if flagCayendo==true then--Si el flag que indica que rayman esta cayendo esta activado
     ...
     if personaje.y>H+personaje.contentHeight then--Si rayman se sale de la pantalla
       composer.gotoScene("liberarGlobox.nivelGlobox3")--Lanzamos la escena del nivel 2 del juego liberar a globox
     end
  end
...
function scene:create( event )
    ...
    tablaSonido = { 
            musicaFondo=audio.loadStream("sonido/musicaInicioNivelGlobox3.wav"),
  ...
  for i=1,3 do
     fondo[i]=soporte:CrearObjeto("graficos/liberarGlobox/inicioNivel3/fondoInicioNivel3 ("..i..").png",W,H,0,0,W*(i-1),0)
     sceneGroup:insert(fondo[i])
    end
    --Creamos los electrones
     electroons=soporte:CrearAnimacionElectroons(centroX+270,centroY-140)
    --Creamos la animacion de rayman 
    personaje=soporte:CrearAnimacionRay(0,centroY-280)
...
function scene:show( event )
    ...
    elseif ( phase == "did" ) then
      -- Called when the scene is now on screen
       ...
       --Reproducimos la musica de fondo con el volumen al 70%
